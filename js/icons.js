const icons = (() => {
    ...
    const iconPaths = {
        ...
        // 타이머/스톱워치 (기존 timer.js 인라인 SVG와 동일한 모양)
        timer: '<path d="M6 2h12v6l-4 4 4 4v6H6v-6l4-4-4-4V2z"/>',
        ...
        // Google Drive 동기화 관련
        cloud: '<path d="M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z"/>', // 클라우드
        user: '<path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>', // 사용자
        'log-in': '<path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10,17 15,12 10,7"/><line x1="15" y1="12" x2="3" y2="12"/>', // 로그인
        'log-out': '<path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16,17 21,12 16,7"/><line x1="21" y1="12" x2="9" y2="12"/>', // 로그아웃
        'refresh-cw': '<polyline points="23,4 23,10 17,10"/><polyline points="1,20 1,14 7,14"/><path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4l-4.64 4.36A9 9 0 0 1 3.51 15"/>', // 동기화
        'download-cloud': '<polyline points="8,17 12,21 16,17"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.88 18.09A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.29"/>', // 초기 동기화
    ...
    const get = (iconName, size = 24, strokeWidth = 1.5) => {
        ...
        if (!pathData) {
            ...
            // 기본 아이콘으로 close 반환 (SVG 전체를 반환하도록 수정)
            return createSVG(iconPaths.close, size, strokeWidth);
        }

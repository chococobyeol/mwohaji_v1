...
*   사용자는 할 일의 텍스트 부분을 클릭하여 수정 모드로 진입할 수 있다.
*   수정 후 'Enter' 키를 누르거나 포커스를 잃으면 내용이 저장된다.
*   수정 시에도 카테고리를 변경할 수 있다.

**F-04: 할 일 삭제 (Delete)**
*   각 할 일 항목에는 '삭제' 버튼이 존재하며, 클릭 시 해당 할 일 데이터가 목록에서 즉시 삭제된다.
...
*   **내보내기 (Export):**
    *   '내보내기' 버튼을 클릭하면 `localStorage`에 저장된 모든 할 일 데이터가 지정된 `.txt` 형식으로 변환된다.
    *   `mwohaji_backup_YYYY-MM-DD.txt` 와 같은 이름의 파일로 사용자 컴퓨터에 다운로드된다.
    *   파일 형식: `[상태] 내용 @cat:카테고리 @notify:YYYY-MM-DD HH:mm @rec:규칙` (e.g., `[ ] 주간 보고서 작성 @cat:업무 @notify:2024-05-24 09:00 @rec:weekly`)
*   **가져오기 (Import):**
    *   '가져오기' 버튼을 클릭하면 파일 선택창이 나타난다.
...
*   **알림 시점 및 동작:**
    *   애플리케이션이 탭에 열려 있는 동안, **주기적으로(예: 1분마다) 시간을 자동으로 확인**한다.
    *   이 확인 작업은 해당 탭이 화면에 보이지 않는 **백그라운드 상태일 때도 동일하게 수행된다.**
...
**F-08: 카테고리 관리 (Manage Categories) - 신규**
*   **카테고리 생성:** 사용자는 설정 메뉴를 통해 새로운 카테고리를 추가할 수 있다.
*   **카테고리 수정:** 기존 카테고리의 이름을 변경할 수 있다. 이름이 변경되면 해당 카테고리를 사용하던 모든 할 일에 즉시 반영된다.
*   **카테고리 삭제:** 카테고리를 삭제할 수 있다. 삭제 시 해당 카테고리에 속해있던 할 일들은 '일반' 카테고리로 자동 변경된다. '일반' 카테고리는 삭제할 수 없다.

#### **5. 기술 사양 (Technical Specifications)**

#### **5.1. 기술 스택**
*   **코어 기술:** Vanilla JavaScript (ES6+)
*   **마크업:** HTML5
...
*   **개별 할 일 아이템 (`<TodoItem />`)**
    ...
    *   **`할 일 텍스트`**: 할 일의 내용.
        ...
        *   **클릭 시:** 텍스트가 입력 필드로 변환되어 수정 모드로 진입.
    *   **`메타 정보`**: 할 일 텍스트 하단에 작은 글씨나 아이콘으로 표시
        *   카테고리: `업무` (텍스트 태그 형태)
        *   알림 시각: `@notify: 2024-05-24 09:00`
        *   반복: `(매주)` 또는 반복 아이콘 `↺`
    *   **`[소리 토글]`**: 소리 알림 On/Off 토글 버튼.
    *   **`[삭제 버튼]`**: 아이템 우측 끝에 'X' 또는 '휴지통' 아이콘 형태로 표시. 마우스를 올렸을 때만 나타나 화면을 깔끔하게 유지.
